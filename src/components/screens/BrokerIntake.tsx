...
import { cases as allCases, getAccountById, formatDate } from "@/lib/mock-data";

const brokerCases = allCases.filter((c) => c.type === "Broker Intake");
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-1">
          ...
          <button
            onClick={() => setMode("old")}
            className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${
              mode === "old" ? "bg-white text-red-600 shadow-sm" : "text-slate-500"
            }`}
          >
            📧 Old Way
          </button>
      ...
      {mode === "old" ? (
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="bg-white rounded-xl border border-red-200 p-5 relative">
            <div className="absolute -top-3 left-4 bg-red-50 text-red-600 text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wide">
              Current State
            </div>
            <div className="space-y-3 mt-2">
              <div className="border border-slate-200 rounded-lg p-3 bg-slate-50">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-medium text-slate-600">From: broker@ebi-brokers.com</span>
                </div>
                <p className="text-xs text-slate-500 italic">
                  &quot;Hey team, got a deal for you. Client is some manufacturing company in Ohio, I think they use about 75,000 MWh. Can you price this up? They want power only. Attached is their latest bill somewhere. Need it by Friday. Thanks!&quot;
                </p>
              ...
              <div className="border border-slate-200 rounded-lg p-3 bg-slate-50">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-sm">📧</span>
                  <span className="text-xs font-medium text-slate-600">RE: broker@ebi-brokers.com</span>
                </div>
                <p className="text-xs text-slate-500 italic">
                  &quot;Following up on this — did you get my email from last week? The client is getting antsy. Also, I forgot to mention they want gas too.&quot;
                </p>
              </div>
              <div className="border border-slate-200 rounded-lg p-3 bg-slate-50">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-sm">📧</span>
                  <span className="text-xs font-medium text-slate-600">FWD: broker@ebi-brokers.com</span>
                </div>
                <p className="text-xs text-slate-500 italic">
                  &quot;Here is the bill I forgot to attach. Also the company name is GlobalTech Industries. Sorry about that.&quot;
                </p>
              </div>
            </div>
            <div className="mt-4 bg-red-50 rounded-lg p-3">
              <p className="text-xs font-medium text-red-700">Result: 3 emails, 5 days, still missing data</p>
              <p className="text-[11px] text-red-600 mt-1">No case created · No opportunity linked · No SLA tracking · BDSS manually triaging inbox</p>
            </div>
          </div>

          <div className="bg-white rounded-xl border border-teal/30 p-5 relative">
            <div className="absolute -top-3 left-4 bg-teal/10 text-teal-dark text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wide">
              Future State
            </div>
            ...
            <div className="mt-4 bg-teal/5 rounded-lg p-3">
              ...
              <p className="text-[11px] text-teal mt-1">Case created · Opportunity linked · SLA clock started · Zero email overhead</p>
            </div>
          </div>
        </div>
      ) : (
        /* NEW WAY — Structured Form */
        <div className="grid grid-cols-3 gap-6">
            ...
            {!submitted ? (
              <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-navy focus:outline-none focus:ring-1 focus:ring-navy/20"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-navy focus:outline-none focus:ring-1 focus:ring-navy/20"
                      placeholder="e.g., 1234 Industrial Blvd, Columbus, OH"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    ...
                    >
                      ...
                      <option>ComEd (PJM)</option>
                      <option>AEP (PJM)</option>
                      <option>DTE (MISO)</option>
                      <option>ERCOT</option>
                      <option>Eversource (ISO-NE)</option>
                      <option>NYISO</option>
                      <option>CAISO</option>
                      <option>Nicor Gas</option>
                      <option>Columbia Gas</option>
                      <option>Other</option>
                    </select>
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-slate-600 mb-1">
                      Broker Margin <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="text"
                      required
                      value={formData.brokerMargin}
                      onChange={(e) => setFormData({ ...formData, brokerMargin: e.target.value })}
                      className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm focus:border-navy focus:outline-none focus:ring-1 focus:ring-navy/20"
                      placeholder="e.g., $0.003/kWh"
                    />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-slate-600 mb-1">
                      Usage / Invoice Upload
                    </label>
                ...
                <button
                  type="submit"
                  className="w-full text-center text-sm font-medium text-white bg-navy rounded-lg py-2.5 hover:bg-navy-dark transition-colors"
                >
                  Submit Deal Intake →
                </button>
              </form>
              ...
              <div className="bg-white rounded-xl border border-teal/30 p-8 text-center">
                ...
                <p className="text-sm text-slate-500 mt-2">
                  Case <span className="font-mono font-medium text-navy">#500C-2026-0892</span> has been created.
                </p>
                <div className="mt-4 bg-slate-50 rounded-lg p-4 text-left max-w-md mx-auto">
                  <div className="space-y-1.5 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-navy">Auto-generated</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Assigned To:</span>
                      <span className="font-medium">BDSS Team Alpha</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Estimated SLA:</span>
                      <span className="font-medium text-teal">24 hours</span>
                    </div>
                  </div>
                </div>
                <button
                  onClick={() => { setSubmitted(false); setFormData({ customerName: "", serviceAddress: "", utility: "", product: "Power", annualVolume: "", brokerMargin: "", usageNotes: "" }); }}
                  className="mt-4 text-sm font-medium text-navy hover:text-navy-dark"
                >
                  Submit Another Deal →
                </button>
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              {brokerCases.map((c) => {
                const acct = getAccountById(c.accountId);
                return (
                  <div key={c.id} className="bg-white rounded-lg border border-slate-200 p-3">
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className="text-[11px] text-slate-500 mt-0.5">{acct?.name} · Age: {c.age}d</p>
                      </div>
                    ...
                    {c.assignedBDSS && (
                      <p className="text-[10px] text-slate-400 mt-1">→ {c.assignedBDSS}</p>
                    )}
                    {c.status === "New" && (
                      <p className="text-[10px] text-red-500 mt-1">⚠ Unassigned — awaiting triage</p>
                    )}
                  </div>
                );
              })}
